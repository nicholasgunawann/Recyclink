...
@endphp
<div class="relative group flex items-center hover:bg-gray-50 transition-colors" data-conv-id="{{ $conv->id }}">
    <a href="{{ route('conversations.show', $conv->id) }}" class="flex items-center gap-4 px-5 py-4 flex-1 min-w-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            @endif
            @if($lastMsg)
            <p class="text-xs text-gray-500 truncate">{{ $lastMsg->message_text ?? '📷 Gambar' }}</p>
            @else
    ...
    {{-- Delete (hide) button: appears on hover --}}
    <button type="button"
        onclick="hideConversation({{ $conv->id }}, this)"
        class="opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mr-4 p-2 text-gray-300 hover:text-red-400 rounded-lg hover:bg-red-50"
