...
@section('title', 'Edit Listing Limbah - Recyclink')
@section('header_title', 'Edit Listing')
...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h3 class="text-2xl font-bold text-gray-900">Edit Listing Limbah</h3>
            <p class="text-gray-600 mt-1">Ubah detail limbah yang Anda jual.</p>
        </div>
    </div>

    <form id="listing-form" action="{{ route('seller.listings.update', $wasteListing) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="p-6 md:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-1">
                    <label class="block font-bold text-gray-900 mb-1">Jenis Limbah <span class="text-red-500">*</span></label>
                    <p class="text-xs text-gray-500">Pilih kategori limbah yang sesuai.</p>
                </div>
                <div class="md:col-span-2">
                    <div class="mb-4">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $wasteListing->title) }}" placeholder="Contoh: Minyak Jelantah Bersih" minlength="5" maxlength="150" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
                    ...
                    <div>
                        ...
                        <select id="category_id" name="category_id" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $wasteListing->category_id) == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" minlength="10" placeholder="Contoh: Minyak jelantah dari usaha gorengan, disaring secara rutin..." class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>{{ old('description', $wasteListing->description) }}</textarea>
                ...
                <div class="md:col-span-2 space-y-4">
                    <div class="flex gap-4">
                        <div class="flex-1">
                            ...
                            <input type="number" step="0.01" min="0.01" id="quantity" name="quantity" value="{{ old('quantity', $wasteListing->quantity) }}" placeholder="Misal: 20" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
                        ...
                        <div class="flex-1">
                            ...
                            <select id="unit" name="unit" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
                                <option value="kg" {{ old('unit', $wasteListing->unit) == 'kg' ? 'selected' : '' }}>Kg (Kilogram)</option>
                                <option value="liter" {{ old('unit', $wasteListing->unit) == 'liter' ? 'selected' : '' }}>Liter</option>
                                <option value="pcs" {{ old('unit', $wasteListing->unit) == 'pcs' ? 'selected' : '' }}>Pcs / Buah</option>
                                <option value="karung" {{ old('unit', $wasteListing->unit) == 'karung' ? 'selected' : '' }}>Karung</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="price_per_unit" name="price_per_unit" value="{{ old('price_per_unit', $wasteListing->price_per_unit) }}" min="1000" placeholder="4000" class="w-full pl-10 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2">
                    <textarea id="address" name="address" rows="3" minlength="5" placeholder="Jl. Tembalang Raya No. 20..." class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>{{ old('address', $wasteListing->address) }}</textarea>
                    ...
                    <div class="mt-3">
                        ...
                        <input type="text" id="city" name="city" value="{{ old('city', $wasteListing->city) }}" placeholder="Semarang" class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-brand focus:border-brand block p-3 transition-colors" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-1">
                    ...
                    <p class="text-xs text-gray-500">Tambahkan foto baru untuk mengganti yang lama (Max 2MB per file).</p>
                </div>
                <div class="md:col-span-2">
                    ...
                    @if($wasteListing->images && $wasteListing->images->count() > 0)
                        <div class="mt-4">
                            <p class="text-xs font-semibold text-gray-600 mb-2">Foto Saat Ini:</p>
                            <div class="flex gap-3 overflow-x-auto pb-2">
                                @foreach($wasteListing->images as $img)
                                    <img src="{{ asset('storage/' . $img->image_url) }}" class="h-20 w-20 object-cover rounded-xl border border-gray-200 flex-shrink-0" alt="Foto">
                                @endforeach
                            </div>
                        </div>
                    @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initial setup for edit values
        if (titleInput) titleCounter.textContent = `${titleInput.value.length} / 150`;
        if (descInput) descCounter.textContent = `${descInput.value.length} karakter`;
        if (priceInput && parseInt(priceInput.value) > 0) {
            const unitVal = document.getElementById('unit')?.value || 'kg';
            pricePreview.textContent = `Preview: Rp ${parseInt(priceInput.value).toLocaleString('id-ID')} / ${unitVal}`;
