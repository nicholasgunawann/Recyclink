...
<body class="bg-white flex min-h-screen antialiased">
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 overflow-y-auto">
        <div class="max-w-md w-full mt-10 lg:mt-0">
            ...
            <form action="{{ route('password.email') }}" method="POST" class="space-y-6" data-turbo="false">
                @csrf
                <div>
                ...
                <button type="submit"
                        class="w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-sm text-base font-bold text-white bg-brand hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand transition-colors cursor-pointer">
                    Kirim Tautan Reset
                </button>
            </form>
